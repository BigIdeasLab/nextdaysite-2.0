...
export function CheckoutFlow({ plans, defaultPlanId }: CheckoutFlowProps) {
  ...
  const summary = useMemo(() => {
    ...
    const base = billingCycle === "monthly" ? selectedPlan.monthly_price : selectedPlan.yearly_price;
    const hosting = includeHosting ? (billingCycle === "monthly" ? HOSTING_MONTHLY : HOSTING_YEARLY) : 0;
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!email) {
      ...
      setErrorMessage("Please provide a contact email so we can share your payment link.");
      return;
    }
    ...
    try {
      ...
      console.error(error);
      setErrorMessage("We were unable to start the checkout session. Please try again.");
      setSubmissionState("error");
    }
  }
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-8">
            ...
            <div className="inline-flex self-start rounded-full border border-foreground/10 bg-foreground/5 p-1 text-xs font-medium text-foreground/70">
              <button
                ...
                className={`rounded-full px-3 py-1 transition ${
                  billingCycle === "monthly" ? "bg-background text-foreground shadow" : ""
                }`}
              ...
              <button
                type="button"
                className={`rounded-full px-3 py-1 transition ${
                  billingCycle === "yearly" ? "bg-background text-foreground shadow" : ""
                }`}
              ...
              return (
                <label
                  ...
                  className={`flex cursor-pointer flex-col gap-3 rounded-2xl border p-4 transition ${
                    isSelected ? "border-foreground bg-foreground text-background" : "border-foreground/10 bg-background"
                  }`}
                >
                  <input
                    className="hidden"
                  ...
                  <span className="text-sm font-semibold uppercase tracking-wide">
                    {plan.name}
                    {plan.is_featured ? <span className="ml-2 rounded-full bg-background/15 px-2 py-0.5 text-xs">Popular</span> : null}
                  </span>
                  <span className="text-2xl font-semibold">{formatCurrency(price)}</span>
                  <p className={`text-sm ${isSelected ? "text-background/80" : "text-foreground/70"}`}>{plan.summary}</p>
                  <ul className={`space-y-1 text-xs ${isSelected ? "text-background/70" : "text-foreground/60"}`}>
        ...
        <section className="flex flex-col gap-4 rounded-2xl border border-foreground/10 bg-background p-6 shadow-sm shadow-foreground/5">
          <header className="flex flex-col gap-2">
            ...
            <p className="text-sm text-foreground/70">
              Bundle managed hosting to keep your NextDaySite live with automatic updates and monitoring.
            </p>
          </header>
          <div className="flex items-start justify-between gap-4 rounded-2xl border border-foreground/10 bg-foreground/5 p-4">
            <div className="flex flex-col gap-1">
              <h3 className="text-sm font-semibold text-foreground">Managed hosting</h3>
              <p className="text-xs text-foreground/70">Deployed on Vercel with uptime monitoring and SSL.</p>
            ...
            <button
              ...
              className={`rounded-full px-4 py-2 text-xs font-semibold transition ${
                includeHosting ? "bg-foreground text-background" : "border border-foreground/30 bg-background text-foreground"
              }`}
        ...
        <section className="flex flex-col gap-4 rounded-2xl border border-foreground/10 bg-background p-6 shadow-sm shadow-foreground/5">
          <header className="flex flex-col gap-2">
            ...
            <p className="text-sm text-foreground/70">Tell us where to send onboarding and draft deliverables.</p>
          </header>
          ...
          <p className="rounded-xl bg-foreground/5 p-4 text-xs text-foreground/60">
            Card details will be collected on the secure Stripe checkout page after you confirm this order.
          </p>
          {errorMessage ? <p className="text-sm font-medium text-rose-500">{errorMessage}</p> : null}
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-xs text-foreground/50">
              By continuing you agree to the NextDaySite terms and recurring billing policy.
            </p>
            <button
              type="submit"
            ...
            >
              {submissionState === "success" ? "Checkout link sent" : submissionState === "submitting" ? "Preparing checkout..." : "Continue to payment"}
            </button>
          </div>
        </section>
      </form>
        ...
        {selectedPlan && summary ? (
          <div className="flex flex-col gap-3 text-sm text-foreground/70">
            ...
            <div className="flex items-center justify-between">
              ...
              <span>{includeHosting ? formatCurrency(summary.hosting) : "$0"}</span>
            </div>
            <div className="flex items-center justify-between">
              <span>Tax (7%)</span>
          ...
          <p className="text-sm text-foreground/60">Select a plan to see pricing.</p>
        )}
      ...
      {submissionState === "success" ? (
        <div className="lg:col-span-2">
          <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-sm text-emerald-600">
            A secure payment link is on its way to <span className="font-semibold">{email}</span>. We’ll start prepping your launch assets immediately.
          </div>
        </div>
      ) : null}
    </div>
