...
function renderMetadata(activity: ActivitiesRow) {
  ...
  switch (activity.event_type) {
    case "status_change": {
      const from = typeof metadata.from === "string" ? metadata.from : "previous stage";
      const to = typeof metadata.to === "string" ? metadata.to : "next stage";
      return (
        <p className="text-xs text-foreground/60">
          Transitioned from <span className="font-medium text-foreground/80">{from}</span> to <span className="font-medium text-foreground/80">{to}</span>
        </p>
      );
    }
    case "file_uploaded": {
      const fileName = typeof metadata.file_name === "string" ? metadata.file_name : "new asset";
      return (
        <p className="text-xs text-foreground/60">
          Uploaded <span className="font-medium text-foreground/80">{fileName}</span>
        </p>
      );
    }
    case "invoice_sent": {
      const invoiceId = typeof metadata.invoice_id === "string" ? metadata.invoice_id : "invoice";
      return (
        <p className="text-xs text-foreground/60">
          Sent <span className="font-medium text-foreground/80">{invoiceId}</span> for approval
        </p>
      );
    }
    case "note_added": {
    ...
    case "message_posted": {
      ...
      return (
        <p className="text-xs italic text-foreground/60">“{preview}”</p>
      );
    }
    default:
      return null;
