...
function RevenueTooltip({ active, payload }: { active?: boolean; payload?: any[] }) {
  ...
  return (
    <div className="rounded-xl border border-foreground/10 bg-background/95 p-3 text-xs shadow-lg shadow-foreground/10">
      ...
      {revenueEntry ? <p className="text-foreground/70">Revenue: {formatCurrency(revenueEntry.value)}</p> : null}
      {forecastEntry ? <p className="text-foreground/60">Forecast: {formatCurrency(forecastEntry.value)}</p> : null}
    </div>
  );
}
  ...
  return (
    <ResponsiveContainer width="100%" height={280}>
      <LineChart data={data} margin={{ top: 16, right: 24, left: 0, bottom: 8 }}>
        ...
        <XAxis dataKey="month" stroke="rgba(148, 163, 184, 0.7)" fontSize={12} tickLine={false} axisLine={false} />
        <YAxis
        ...
        />
        <Tooltip content={<RevenueTooltip />} cursor={{ stroke: "rgba(148, 163, 184, 0.2)", strokeWidth: 1 }} />
        <Line type="monotone" dataKey="revenue" stroke="var(--foreground)" strokeWidth={2} dot={false} />
        <Line type="monotone" dataKey="forecast" stroke="rgba(148, 163, 184, 0.8)" strokeWidth={2} strokeDasharray="5 5" dot={false} />
      </LineChart>
    </ResponsiveContainer>
  );
}
