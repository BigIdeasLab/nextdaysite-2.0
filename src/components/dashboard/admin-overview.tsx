...
export function AdminOverview() {
  ...
  const metrics = useMemo(() => {
    ...
    const revenueTotal = paidInvoices.reduce((sum, invoice) => sum + invoice.total, 0);
    ...
    const currentActiveCustomers = new Set(projects.map((project) => project.owner_id)).size;
    ...
    const averageCycle = cycleDurations.length
      ? Math.round(cycleDurations.reduce((sum, days) => sum + days, 0) / cycleDurations.length)
      : 0;
  ...
  const revenueChartData = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i -= 1) {
      const seed = new Date(reference.getFullYear(), reference.getMonth() - i, 1);
      months.push(`${MONTH_LABELS[seed.getMonth()]} ${seed.getFullYear().toString().slice(-2)}`);
    }

    return months.map((month) => ({ month, revenue: map.get(month)?.revenue ?? 0, forecast: map.get(month)?.forecast ?? 0 }));
  }, [invoices]);
  ...
  const planDistribution = useMemo(() => {
    ...
    return plans.map((plan) => ({ name: plan.name, value: counts.get(plan.id) ?? 0 })).filter((slice) => slice.value > 0);
  }, [plans, users]);
  ...
  const topCustomers = useMemo(() => {
    const revenueByCustomer = invoices.reduce<Record<string, number>>((acc, invoice) => {
      if (invoice.status !== "paid") {
        return acc;
      }
    ...
    return Object.entries(revenueByCustomer)
      .map(([userId, total]) => ({
        userId,
        total,
        projects: projects.filter((project) => project.owner_id === userId).length,
      }))
    ...
    <div className="flex flex-col gap-10">
      <header className="flex flex-col gap-2">
        ...
        <p className="text-sm text-foreground/70 sm:text-base">
          Monitor revenue performance, pipeline health, and customer impact in real time.
        </p>
      </header>
      ...
      <div className="grid gap-6 xl:grid-cols-[2fr_1fr]">
        ...
        <section className="rounded-2xl border border-foreground/10 bg-background p-6 shadow-sm shadow-foreground/5">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-foreground">Plan distribution</h2>
        ...
        <section className="rounded-2xl border border-foreground/10 bg-background p-6 shadow-sm shadow-foreground/5">
          ...
          <ul className="mt-4 space-y-3">
            {topCustomers.map((record) => {
              ...
              return (
                <li key={record.userId} className="flex items-center justify-between rounded-xl bg-foreground/5 p-3 text-sm">
                  <div className="flex flex-col">
                    <span className="font-semibold text-foreground">{customer?.full_name ?? customer?.email ?? "Unknown"}</span>
                    <span className="text-xs text-foreground/60">{record.projects} active projects</span>
                  </div>
                  <span className="text-sm font-semibold text-foreground">{formatCurrency(record.total)}</span>
                </li>
              );
            })}
      ...
      <ActivityTimeline activities={activities} projects={projects} users={users} title="Latest operations" />
    </div>
  );
}
