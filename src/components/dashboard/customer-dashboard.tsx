...
export function CustomerDashboard() {
  ...
  const metrics = useMemo(() => {
    const activeProjects = projects.filter((project) => project.status !== "shipped");
  ...
  return (
    <div className="flex flex-col gap-10">
      <header className="flex flex-col gap-2">
        <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">Project pulse</h1>
      ...
      <ActivityTimeline activities={activities} projects={projects} users={users} />
    </div>
  );
}
