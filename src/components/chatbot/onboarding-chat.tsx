...
const SuggestionButton = ({
  ...
  <button
    ...
    disabled={disabled}
    className='rounded-full border border-gray-600 bg-gray-800 px-4 py-2 text-sm text-white transition-colors hover:bg-gray-700 disabled:cursor-not-allowed disabled:opacity-50'
  >
    {suggestion}
  </button>
)
  ...
  return (
    ...
    >
      ...
      <div className='flex flex-1 flex-col justify-between overflow-hidden rounded-[30px] bg-[#131313] p-5'>
          ...
          <>
            ...
            {showAuthButtons && (
              <div className='flex items-center justify-center gap-4 p-4'>
                <Link
                  href='/signup'
                  className='rounded-[20px] bg-green-600 px-6 py-3 text-lg font-semibold text-white transition-colors hover:bg-green-700'
                >
                  Sign Up to Save
                </Link>
                <Link
                  href='/login'
                  className='rounded-[20px] bg-blue-600 px-6 py-3 text-lg font-semibold text-white transition-colors hover:bg-blue-700'
                >
                  Log In
                </Link>
              </div>
        ...
        {/* Input Area - Render this only when chat has started */}
        <div className='p-4'>
          <form
            onSubmit={(e) => {
              e.preventDefault()
            ...
            className='flex w-full items-center gap-2 rounded-full bg-[#202020] p-2'
          >
            {/* Left Icon Buttons */}
            ...
            <input
              ...
              className='w-full flex-grow bg-transparent text-base text-white/90 outline-none placeholder:text-white/50'
            ...
            <button
              type='submit'
              disabled={
                isLoading || isConversationComplete || !inputValue.trim()
              }
              className='flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-full bg-[#FF8C00] transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50'
            >
              <svg
                width='30'
                height='30'
