          ...
          return (
            <article key={plan.id} className={containerClasses}>
              <div className="flex flex-col gap-2">
                ...
                <p className={isFeatured ? "text-sm text-background/80" : "text-sm text-foreground/70"}>{plan.summary}</p>
              </div>
              <p className="text-3xl font-semibold">{formatCurrency(plan.monthly_price)}</p>
              <ul className={isFeatured ? "space-y-3 text-sm text-background/85" : "space-y-3 text-sm text-foreground/70"}>
