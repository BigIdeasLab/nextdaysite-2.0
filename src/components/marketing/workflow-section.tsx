...
export function WorkflowSection({ id, headline, description, bullets, cards }: WorkflowSectionProps) {
  return (
    <section id={id} className="grid gap-10 md:grid-cols-2">
      <div className="flex flex-col gap-4">
        ...
        <p className="text-base leading-relaxed text-foreground/70 sm:text-lg">{description}</p>
      ...
      <div className="grid gap-6 rounded-3xl border border-foreground/10 bg-foreground/5 p-6 sm:p-10">
        {cards.map((card) => (
          <div key={card.title} className="rounded-2xl bg-background p-6 shadow-sm shadow-foreground/10">
